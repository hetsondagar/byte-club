...
export function CodeTerminal({ slug, initialCode = "", onCodeChange, disabled }: CodeTerminalProps) {
  ...
  const run = async () => {
    ...
    try {
      const data = slug
        ? await apiService.runCode(slug, code)
        : await apiService.runCodeSandbox(code);
